...
then
    ...
    curl -k -X POST -H "Content-Type: application/json" -d '{"source":{"index":"'$INDEX_NAME_INPUT'"},"dest":{"index":"'$INDEX_NAME_OUTPUT'"},"script":{"source":"ctx._source['"'"'src_version'"'"'] = 3; ctx._source['"'"'time'"'"']  = ctx._source.remove('"'"'mqtt_time'"'"');  ctx._source.txInfo.type = ctx._source.txInfo.remove('"'"'modulation'"'"'); ctx._source.txInfo.modulation = [:]; ctx._source.txInfo.modulation.lora = ctx._source.txInfo.remove('"'"'loRaModulationInfo'"'"'); ctx._source.txInfo.modulation.type  = ctx._source.txInfo.remove('"'"'type'"'"'); if (ctx._source.txInfo.modulation.lora.codeRate != null) {ctx._source.txInfo.modulation.lora.codeRate = '"'"'C_'"'"' + ctx._source.txInfo.modulation.lora.codeRate.replace('"'"'/'"'"','"'"'_'"'"');} ctx._source.rxInfo.snr = ctx._source.rxInfo.remove('"'"'loRaSNR'"'"'); ctx._source.rxInfo.gatewayId = ctx._source.rxInfo.remove('"'"'gatewayID'"'"'); ctx._source.rxInfo.uplinkIdText = ctx._source.rxInfo.remove('"'"'uplinkID'"'"');"}}' "https://${HOSTNAME}:9200/_reindex?wait_for_completion=false" -u ${USER}:${PASSWORD}
